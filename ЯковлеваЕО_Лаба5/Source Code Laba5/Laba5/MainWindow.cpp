...
void CMainWindow::InitGraphics()
{
  ...
  m_pLab5 = new CLaba5_Dinamic();
  m_GraphicsObjects.Add(m_pLab5);

  //  запускаем таймер с частотой 25 кадров/сек (1 кадр/40 мс)
  m_Timer.Start(m_hWnd, 1, 40);
}
...
void CMainWindow::OnSize(int w, int h)
{
  ...
  glLoadIdentity();
  gluPerspective(90.0f, fAspect, 0.1, CONST_GRAPH_SCENE_SIZE);
...
LRESULT CMainWindow::OnTimer(UINT /*uMsg*/, WPARAM nId, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
  m_pLab5->TimerTick();
  InvalidateRect(NULL, FALSE);
  return TRUE;  
